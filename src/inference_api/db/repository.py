...
SELECT
    ...
    COALESCE((p.value->>'cached_tokens_per_sec')::float, 0) AS cached_tokens_per_sec,
    COALESCE((p.value->>'total_input_tokens')::bigint, 0) AS total_input_tokens,
    COALESCE((p.value->>'total_output_tokens')::bigint, 0) AS total_output_tokens,
    COALESCE((p.value->>'total_cached_tokens')::bigint, 0) AS total_cached_tokens,
