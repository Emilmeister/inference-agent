...
class SGLangEngine(BaseEngine):
    ...
    def build_docker_args(self, experiment: ExperimentConfig) -> list[str]:
        ...
        if experiment.pipeline_parallel_size > 1:
            serve_args.extend(["--pp-size", str(experiment.pipeline_parallel_size)])
        ...
        if experiment.mem_fraction_static is not None:
            serve_args.extend(["--mem-fraction-static", str(experiment.mem_fraction_static)])
        ...
        if experiment.max_running_requests is not None:
            serve_args.extend(["--max-running-requests", str(experiment.max_running_requests)])

        if experiment.max_prefill_tokens is not None:
            serve_args.extend(["--max-prefill-tokens", str(experiment.max_prefill_tokens)])
        ...
        if experiment.enable_chunked_prefill and experiment.chunked_prefill_size is not None:
            serve_args.extend(["--chunked-prefill-size", str(experiment.chunked_prefill_size)])
        ...
        if experiment.speculative_algorithm:
            serve_args.extend(["--speculative-algorithm", experiment.speculative_algorithm])
